...
import requests
import json
from mongo_connect import dbInsertFile,download_gridfs
...
def insert_users(n_clicks, un, pw, em):
    ...
    if un is not None and pw is not None and em is not None:
        id_count = Users.query.count()+1
        ins = Users_tbl.insert().values(id= id_count, username=un,  password=hashed_password, email=em,)
        conn = engine.connect()
        conn.execute(ins)
        conn.close()
        return [login]
...
def successful(n_clicks, input1, input2):
    ...
    if user:
        if check_password_hash(user.password, input2):
            login_user(user)
            usern = "hello"
            fileID = dbInsertFile("202004-divvy-tripdata.csv",usern)
            download_gridfs(usern, fileID,{})
